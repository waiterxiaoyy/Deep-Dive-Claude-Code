...
export default function ProductionPatternsVisualization() {
  ...
  return (
    <section className="min-h-[520px] space-y-4">
      <h2 className="text-xl font-semibold text-zinc-100">生产级工程模式</h2>
      <div className="rounded-lg border border-zinc-700 bg-zinc-900 p-4">
        <div className="flex flex-col gap-4 lg:flex-row">
          <div className="w-full lg:w-[35%]">
            <div className="mb-3 font-mono text-xs text-zinc-500">工程模式清单</div>
            <div className="space-y-2">
              {["错误重试", "优雅关机", "会话持久化", "A/B 测试", "错误边界", "全景总结"].map((label, i) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-xs ${isActive ? "text-white" : isDone ? "text-emerald-400" : "text-zinc-600"}`}>{label}</span>
          ...
          <div className="w-full lg:w-[65%]">
            <div className="mb-2 font-mono text-xs text-zinc-500">代码模式</div>
            <div className="min-h-[380px] rounded-md border border-zinc-800 bg-zinc-950 p-3">
              {patterns.length > 0 ? patterns.map((p, i) => (
                <motion.div key={`${viz.currentStep}-${i}`} initial={{opacity:0, y:12}} animate={{opacity:1, y:0}}
                  ...
                  <div className="mb-2 text-sm font-semibold text-zinc-100">{p.title}</div>
